...
def index(request):
    ...
    context = {
        "result": request.session.get(
            "result", ""
        ),  # pass the results from result function
        "counter": request.session["counter"],  # count number of attempts
        "rand": request.session["rand"],
        "success": request.session.get(
            "success", 0
        ),  # used with if, if success=1 show submit username form after win
    }
...
def result(request):
    ...
    if request.method == "POST":
        ...
        elif 10 == request.session["counter"] and (
            guess > request.session["rand"] or guess < request.session["rand"]
        ):
...
def submit_username(request):
    if request.method == "POST":
        ...
        user1 = User.objects.create(
            username=request.POST.get("username"), score=request.session["counter"]
        )
    return redirect("play/")
